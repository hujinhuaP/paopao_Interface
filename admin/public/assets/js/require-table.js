    ...
    var Table = {
        ...
        api: {
            ...
            multi: function (action, ids, table, element) {
                ...
                var remark = typeof data.remark !== "undefined" ? (typeof data.remark == 'object' ? $.param(data.remark) : data.remark) : '';
                var options = {url: url, data: {action: action, ids: ids, params: params,remark: remark}};
            ...
            events: {
                operate: {
                    'click .btn-detailone': function (e, value, row, index) {
                        ...
                        Fast.api.open(options.extend.detail_url + (options.extend.detail_url.match(/(\?|&)+/) ? "&ids=" : "/ids/") + row[options.pk], __('Detail'));
                    },
                    ...
                    'click .btn-tipsone': function (e, value, row, index) {
                        e.stopPropagation();
                        var that = this;
                        var top = $(that).offset().top - $(window).scrollTop();
                        var left = $(that).offset().left - $(window).scrollLeft() - 260;
                        ...
                        let confirmMsg = $(that).data('confirmmsg');
                        if(!confirmMsg){
                            confirmMsg = __('Are you sure you want to operate this item?');
                        }
                        var index = Layer.confirm(
                            confirmMsg,
                                {icon: 3, title: __('Warning'), offset: [top, left], shadeClose: true},
                                function () {
                                    var table = $(that).closest('table');
                                    var options = table.bootstrapTable('getOptions');
                                    Table.api.multi(undefined, row[options.pk], table, that);
                                    Layer.close(index);
                                }
                        );
                    }
            ...
            formatter: {
                ...
                datetime: function (value, row, index) {
                    return value ? Moment(parseInt(value) * 1000).format("YYYY-MM-DD HH:mm:ss") : '-';
                },
                date: function (value, row, index) {
                    return value ? Moment(parseInt(value) * 1000).format("YYYY-MM-DD") : '-';
                },
                operate: function (value, row, index) {
                    ...
                    buttons.unshift({name: 'del', icon: 'fa fa-trash', classname: 'btn btn-xs btn-danger btn-delone', title:__('Delete'), text:__('Delete')});
                    buttons.unshift({name: 'edit', icon: 'fa fa-pencil', classname: 'btn btn-xs btn-success btn-editone', title:__('Edit'), text:__('Edit')});
                    buttons.unshift({name: 'detail', icon: 'fa fa-list', classname: 'btn btn-xs btn-info btn-detailone', title:__('Detail'), text:__('Detail')});
                    buttons.unshift({name: 'dragsort', icon: 'fa fa-arrows', classname: 'btn btn-xs btn-primary btn-dragsort', title:__('Dragsort'), text:__('Dragsort')});
                    var html = [];
                    ...
                    $.each(buttons, function (i, j) {
                        ...
                        if ((typeof attr === 'undefined' || attr) || (j.name === 'dragsort' && typeof row[Table.config.dragsortfield] == 'undefined')) {
                            if (['add', 'edit', 'del', 'multi', 'detail'].indexOf(j.name) > -1 && !options.extend[j.name + "_url"]) {
                                return true;
                            }
                            //自动加上ids
