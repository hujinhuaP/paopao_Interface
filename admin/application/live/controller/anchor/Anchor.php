...
use app\live\model\live\AnchorDispatch;
...
use app\live\model\live\UserProfileSetting;
use think\Exception;
...
class Anchor extends Backend
{
    ...
    public function index()
    {
        if ( $this->request->isAjax() ) {
            ...
            $red_flg   = $this->request->get("red_flg", '');
            $where_str = '1=1';
            if ( $hot_flg == 'hot' ) {
                $where_str = 'anchor_hot_man > 0';
            }
            if ( $red_flg == 'red' ) {
                $where_str .= ' AND anchor_hot_time > 0';
            }
    ...
    public function sign( $ids = '' )
    {
        ...
        if ( $this->request->isAjax() ) {
            ...
            if ( $row->save($row->getData()) === FALSE ) {
                ...
                // 添加主播操作日志
                AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_SIGN, $row->user_id, 'Y');
                $this->success();
            }
        }
    ...
    public function cancelsign( $ids = '' )
    {
        ...
        if ( $this->request->isAjax() ) {
            ...
            if ( $row->save($row->getData()) === FALSE ) {
                ...
                // 添加主播操作日志
                AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_SIGN, $row->user_id, 'N');
                $this->success();
            }
        }
    }
    ...
    public function status( $ids = '' )
    {
        ...
        if ( $this->request->isPost() ) {
            ...
            if ( $params[0] == 'anchor_is_show_index' ) {
                $oRedis            = new Redis();
                $new_100_anchor_id = $oRedis->get('new_100_anchor_id');
                if ( $new_100_anchor_id && $row->anchor_id > $new_100_anchor_id ) {
                    $oRedis->del('new_100_anchor_id');
                }
            }
            if ( $row->save() === FALSE ) {
                $this->error($row->getError());
            ...
            switch ( $params[0] ) {
                case 'anchor_is_show_index':
                    // 添加主播操作日志
                    AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_SHOW_INDEX, $row->user_id, $params[1]);
                    break;
                case 'anchor_is_newhot':
                    // 添加主播操作日志
                    AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_NEW_HOT, $row->user_id, $params[1]);
                    break;
                case 'anchor_is_positive':
                    // 添加主播操作日志
                    AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_POSITIVE, $row->user_id, $params[1]);
                    break;
                case 'anchor_is_beauty':
                    // 添加主播操作日志
                    AnchorActionLog::record(AnchorActionLog::ACTION_TYPE_BEAUTY, $row->user_id, $params[1]);
                    break;
                case 'anchor_dispatch_flg':
                    if ( $oldRow->anchor_dispatch_flg != $params[1] ) {
                        if ( $params[1] == 'Y' ) {
                            // 变为新数据 判断
                            $oAnchorDispatch = AnchorDispatch::get([ 'anchor_dispatch_user_id' => $row->user_id ]);
                            if ( !$oAnchorDispatch ) {
                                $oAnchorDispatch                                = new AnchorDispatch();
                                $oAnchorDispatch->anchor_dispatch_user_id       = $row->user_id;
                                $oAnchorDispatch->anchor_dispatch_price         = 20;
                                $oAnchorDispatch->anchor_dispatch_max_day_times = 20;
...
//            $oUser->user_withdraw_ratio = $params['user_withdraw_ratio'];
            if ( $params['anchor_video_cover'] != $oAnchor->anchor_video_cover ) {
                $anchor_check_img_arr      = explode(',', $oAnchor->anchor_check_img);
                $anchor_check_img_arr[0]   = $params['anchor_video_cover'];
                $oAnchor->anchor_check_img = implode(',', $anchor_check_img_arr);
            }
     ...
     * @param string $ids
     */
    public function coverchange( $ids = '' )
    {
        $oAnchor = AnchorModel::where('user_id', $ids)->find();
        $oUser   = User::where('user_id', $ids)->find();
        if ( !$oAnchor || !$oUser )
            $this->error(__('No Results were found'));
        if ( $this->request->isPost() ) {
            $params = $this->request->param('row/a');
            ...
            $oAnchor->anchor_video_cover            = $params['anchor_video_cover'] ?? '';
            $oAnchor->anchor_video                  = $params['anchor_video'] ?? '';
            $oAnchor->anchor_private_title          = $params['anchor_private_title'] ?? '';
            $oAnchor->anchor_tip                    = isset($params['anchor_tip']) ? implode(',', $params['anchor_tip']) : '';
            $oAnchor->anchor_character              = isset($params['anchor_character']) ? implode(',', $params['anchor_character']) : '';
            $oAnchor->anchor_good_topic             = isset($params['anchor_good_topic']) ? implode(',', $params['anchor_good_topic']) : '';
            $oAnchor->anchor_dress                  = isset($params['anchor_dress']) ? implode(',', $params['anchor_dress']) : '';
            $oAnchor->anchor_stature                = isset($params['anchor_stature']) ? implode(',', $params['anchor_stature']) : '';
            $oAnchor->anchor_chat_price             = $params['anchor_chat_price'] ?? 30;
            $oAnchor->anchor_connection_custom_rate = $params['anchor_connection_custom_rate'] ?? '80';
            $oAnchor->anchor_custom_title           = $params['anchor_custom_title'] ?? '';
            if ( $oAnchor->save($oAnchor->getData()) === FALSE ) {
                $this->error($oAnchor->getError());
            } else {
                // 修改个性签名
                $userChange = FALSE;
                if ( $params['user_intro'] != $oUser->user_intro ) {
                    $userChange        = TRUE;
                    $oUser->user_intro = $params['user_intro'];
                }
                if ( $params['user_wechat'] != $oUser->user_wechat ) {
                    $userChange         = TRUE;
                    $oUser->user_wechat = $params['user_wechat'];
                }
                if ( $params['user_wechat_price'] != $oUser->user_wechat_price ) {
                    $userChange               = TRUE;
                    $oUser->user_wechat_price = $params['user_wechat_price'];
                }

                if ( $params['user_v_wechat'] != $oUser->user_v_wechat ) {
                    $userChange           = TRUE;
                    $oUser->user_v_wechat = $params['user_v_wechat'];
                }
                if ( $params['user_v_wechat_price'] != $oUser->user_v_wechat_price ) {
                    $userChange                 = TRUE;
        ...
        $profileList = UserProfileSetting::select();
        $saveKeyArr  = [
            'anchor_tip',
            'anchor_character',
            'anchor_good_topic',
        ...
        foreach ( $profileList as $item ) {
            $selectData = explode(',', $item->profile_select);
            $saveData   = [];
            foreach ( $selectData as $itemData ) {
                $saveData[ $itemData ] = $itemData;
            }
            if ( in_array($item->profile_key, $saveKeyArr) ) {
                $this->view->assign([
                    $item->profile_key             => $saveData,
                    $item->profile_key . '_exists' => explode(',', $oAnchor->{$item->profile_key})
                ]);
            }
        }
    ...
    public function examine( $ids = 0 )
    {
        ...
        if ( $this->request->isAjax() ) {
            $params = $this->request->param('row/a');
            if ( isset($params['anchor_is_examine']) && array_key_exists($params['anchor_is_examine'], $app_arr) ) {
                $row->anchor_is_examine = $params['anchor_is_examine'];
                $row->save();
            }
            $this->success();
        }
