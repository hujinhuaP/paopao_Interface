...
class Reportlog extends Backend
{
    ...
    {
        $row = AnchorReportLog::get($ids);
        if (!$row)
            $this->error(__('No Results were found'));

        $images = $row->anchor_report_images;

        $imagesArr = $images ? explode(',',$images) : [];

        $reportUser   = User::get($row->user_id);
        $reportedUser = User::get($row->anchor_user_id);
