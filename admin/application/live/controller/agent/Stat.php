...
class Stat extends Backend {
    public function _initialize() {
        parent::_initialize();
        $this->model = model('live.agent_daily_stat');
    }
    ...
    public function index() {
        if ($this->request->isAjax()) {
            $where_str = '1=1';
            $only_has_register = $this->request->get("only_has_register", '');
            if($only_has_register == '1'){
                $where_str = 'agent_daily_stat.total_register_count > 0';
            }
            list($where, $sort, $order, $offset, $limit) = $this->buildparams('agent_id', true);
            $total  = $this->model->where($where_str)->where($where)->where("agent.status = 'Y'")->with('Agent')->order($sort, $order)->count();
            $list   = $this->model->where($where_str)->where($where)->where("agent.status = 'Y'")->with('Agent')->order($sort, $order)->limit($offset, $limit)->select();
            $result = [
                "total" => $total,
                "rows"  => $list
            ];
    ...
    public function income($ids = '')
    {
        ...
        $agent = AgentModel::get($row->agent_id);
        if ($this->request->isAjax()) {
            $this->model = model('live.agent_water_log');
            $agent_id = $row->agent_id;
            $stat_time = $row->stat_time;
            $start = $stat_time;
