    ...
    public function add()
    {
        ...
        if ( $this->request->isPost() ) {
            ...
            if ( !$row->save($row->getData()) ) {
                ...
                $appItem = NULL;
                if ( $app_id ) {
                    $appItem = model('live.app_list')::get($app_id);
                }
                if ( $appItem ) {
                    // 生成关联的应用市场数据  1个一级代理 加上多个二级代理
                    $inviteCodePrefix = $appItem->app_flg;
                    $firstAgentInvite = $inviteCodePrefix . 'FAgent';
                    $existData        = Agent::get([ 'invite_code' => $firstAgentInvite ]);
                    if ( $existData ) {
                        $row->rollback();
                        $this->error('该APP已经有关联的代理商数据了');
                    }
                    $agent                                = new Agent();
                    $agent->invite_code                   = $firstAgentInvite;
                    $agent->nickname                      = $params['realname'];
                    $agent->account                       = $params['account'];
                    $agent->auth_key                      = md5($params['password'] . $firstAgentInvite);
                    ...
                    $agentArr  = [];
                    foreach ( $markSuArr as $markKey => $markItem ) {
                        $itemInviteCode = strtoupper($inviteCodePrefix . $markKey);
                        $agentArr[] = [
                            'invite_code'  => $itemInviteCode,
                            'nickname'     => $appItem->app_name . $markItem,
                            'account'      => $params['account'] . $markKey,
                            'auth_key'     => md5($params['password'] . $itemInviteCode),
                            'first_leader' => $agent->id,
                            'level'        => 2,
                            'status'       => 'Y',
                            'avatar'       => 'http://api.sxypaopao.com/assets/images/logo.png',
                        ];
                    }
                    $agent = new Agent();
                    if ( $agent->saveAll($agentArr) === FALSE ) {
                        $row->rollback();
                        $this->error($agent->getError());
                    }
