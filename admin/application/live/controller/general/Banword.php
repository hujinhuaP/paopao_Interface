    ...
    public function add()
    {
        if ($this->request->isPost())
        {
            ...
            foreach ($params['banword_content'] as $value) {
                ...
                if (BanwordModel::where(['banword_content' => $value,'banword_location' => $params['banword_location']])->find()) {
                    continue;
                }

                $row = new BanwordModel();
                $row->banword_content = $value;
                $row->banword_location = $params['banword_location'];
                $row->validate(
                    [
                        'banword_content' => 'require',
                        'banword_location' => 'require',
                    ],
                    [
                        'banword_content.require' =>  __('Parameter %s can not be empty', ['banword_content']),
                        'banword_location.require' =>  __('Parameter %s can not be empty', ['banword_location']),
                    ]
                );
            ...
            $oRedis->rm('_PHCRcaches_banword_arr:'.$this->banword_location);
            $this->success();
        }
