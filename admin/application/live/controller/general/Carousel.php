...
class Carousel extends Backend
{
    ...
    public function index()
    {
        ...
        $this->view->assign("row_app", $oAppList);
    ...
    public function add()
    {
        ...
        if ( $this->request->isPost() ) {
            ...
            $row->carousel_title       = $params['carousel_title'];
            $row->carousel_app_id      = $params['carousel_app_id'];
            $row->carousel_and_show    = $params['carousel_and_show'];
            $row->carousel_ios_show    = $params['carousel_ios_show'];
            $row->carousel_target_type = $params['carousel_target_type'];
            $row->is_ad                = $params['is_ad'];
    ...
    public function sort()
    {
        if ( $this->request->isPost() ) {
            ...
            foreach ( $ids as $k => $id ) {
                CarouselModel::where('carousel_id', $id)->update([
                    'carousel_sort'        => $k + 1,
                    'carousel_update_time' => time()
                ]);
            }
            $this->success();
        }
