...
use app\live\model\live\UserCashLog;
use think\Log;
use think\Config;
use think\Exception;
...
class Withdraw extends Backend
{
    ...
    public function edit($ids = null)
    {
        ...
        if ( $oUser->user_invite_user_id ) {
            ...
            if ( !$oInviteUser ) {
                $oInviteUser = null;
            } else if ( $oInviteUser->user_is_anchor == 'Y' ) {
                $invite_withdraw_rate = Kv::getValue(Kv::INVITE_ANCHOR_WITHDRAW_RADIO);
            } else {
                $invite_withdraw_rate = Kv::getValue(Kv::INVITE_WITHDRAW_RADIO_CASH);
                $inviteIsAnchor       = FALSE;
            }
        ...
        if ( $this->request->isPost() ) {
            ...
            try {
                ...
                if($params['user_withdraw_log_check_status'] == 'N'){
                    $row->user_withdraw_way = 0;
                }
                ...
                if($params['user_withdraw_log_check_status'] == 'Y'){
                ...
                switch ( $row->user_withdraw_log_check_status ) {
                    ...
                    case 'Y':
                        if ( isset($params['invite_reward']) && $params['invite_reward'] > 0 ) {
                            ...
                            if ( $inviteIsAnchor ) {
                                ...
                                //如果通过 并且有邀请用户奖励大于0 则需要给邀请用户加“现金”
                                $oUserCashLog = new UserCashLog();
                                $data         = [
                                    'user_id'             => $oInviteUser->user_id,
                                    'user_current_amount' => $oInviteUser->user_cash,
                                    'user_last_amount'    => $oInviteUser->user_cash + $invite_reward,
                                    'consume_category'    => UserCashLog::CATEGORY_WITHDRAW,
                                    'consume'             => $invite_reward,
                                    'remark'              => '邀请主播提现奖励',
                                    'flow_id'             => $row->user_withdraw_log_id,
                                    'flow_number'         => $row->user_withdraw_log_number,
                                ...
                                if ( $oUserCashLog->save($data) === FALSE ) {
                                    $row->getQuery()->rollback();
                                    $this->error($oUserCashLog->getError());
                                }
                                $oInviteUser->user_cash       += $invite_reward;
                                $oInviteUser->total_user_cash += $invite_reward;
                                if ( $oInviteUser->save() === FALSE ) {
                                    $row->getQuery()->rollback();
                                    $this->error($oInviteUser->getError());
                                }
                                $oRedis = new Redis();
                                $redisKey = 'user:today:cash:'. date('Y-m-d') . ':'.$oInviteUser->user_id;
                                $oRedis->incrByFloat($redisKey,$invite_reward);
                            }
                        }
                        break;
                    ...
                    default:
                        if($row->dot_type == 'dot'){
                        ...
                        }elseif($row->dot_type == 'cash'){
                            $oUser           = User::get($row->user_id);
                            $oUser->user_cash += $row->user_dot;
                            if ( $oUser->save() === FALSE ) {
                                $row->getQuery()->rollback();
                                $this->error($oUser->getError());
                            }
                            ...
                            $data            = [
                                ...
                                'user_current_amount' => $oUser->user_dot,
                                'user_last_amount'    => $oUser->user_dot - $row->user_dot,
                                'consume_category' => UserCashLog::CATEGORY_WITHDRAW_BACK,
                                'consume'             => $row->user_dot,
                                'remark'              => '提现退回现金',
                                'flow_id'             => $row->user_withdraw_log_id,
                                'flow_number'         => $row->user_withdraw_log_number,
        ...
        $this->view->assign("inviteIsAnchor", $inviteIsAnchor);
