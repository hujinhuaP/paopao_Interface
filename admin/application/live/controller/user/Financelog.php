...
class Financelog extends Backend
{
    ...
    public function coin()
    {
        $coinFlg              = [
            ...
            UserConsumeCategory::CASH_EXCHANGE,
            UserConsumeCategory::GUARD_PAY,
            UserConsumeCategory::VIP_RECHARGE_REWARD,
            UserConsumeCategory::POSTS_GIFT_PAY,
            UserConsumeCategory::USER_LEVEL_REWARD,
            UserConsumeCategory::DAILY_TASK,
            UserConsumeCategory::ONCE_TASK,
            UserConsumeCategory::CHAT_GAME_PAY,
            UserConsumeCategory::WECHAT_PAY,
            UserConsumeCategory::POSTS_PAY,
        ];
...
//            $total  = $this->model::where('user_amount_type',UserFinanceLogModel::AMOUNT_COIN)->where($where)->with('User,TargetUser')->order($sort, $order)->count();
            $total        = 1000;
            $list         = $this->model::where('user_amount_type', UserFinanceLogModel::AMOUNT_COIN)->where($where)->order($sort, $order)->limit($offset, $limit)->select();
    ...
    public function dot()
    {
        $coinFlg              = [
            ...
            UserConsumeCategory::GUARD_GET,
            UserConsumeCategory::CHAT_GAME_INCOME,
            UserConsumeCategory::ANCHOR_POSTS_INCOME,
            UserConsumeCategory::PHOTOGRAPHER_POSTS_INCOME,
            UserConsumeCategory::WECHAT_INCOME,
            UserConsumeCategory::PHOTOGRAPHER_WECHAT_INCOME,
            UserConsumeCategory::POSTS_GIFT_INCOME,
            UserConsumeCategory::ANCHOR_DAILY_TASK_TASK,
            UserConsumeCategory::DAILY_VIDEO_CHAT_REWARD,
        ];
        $oUserConsumeCategory = UserConsumeCategory::all($coinFlg);
        ...
        if ( $this->request->isAjax() ) {
            list($where, $sort, $order, $offset, $limit) = $this->buildparams();
//            $total  = $this->model::where('user_amount_type',UserFinanceLogModel::AMOUNT_DOT)->where($where)->with('User,TargetUser')->order($sort, $order)->count();
            $total     = 1000;
            $list      = $this->model::where('user_amount_type', UserFinanceLogModel::AMOUNT_DOT)->where($where)->order($sort, $order)->limit($offset, $limit)->select();
    ...
    public function detail($ids = '')
    {
        ...
        $oUser       = UserModel::get($row->user_id);
        $oTargetUser = UserModel::get($row->target_user_id);
        $this->view->assign("row", $row);
        $this->view->assign("oUser", $oUser);
        $this->view->assign("oTargetUser", $oTargetUser);
        return $this->view->fetch();
    }
