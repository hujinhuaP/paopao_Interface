            ...
            try {
                ...
                $bBool = $oUserBudan->validate(
                    ...
                    [
                        ...
                        'remark.require'            => __('Parameter %s can not be empty', [ __('Remark') ]),
                ...
                switch ( $params['user_budan_type'] ) {
                    case 'cash':
                        $nLastAmount          = $row->user_cash;
                        $row->user_cash       += $data['user_budan_amount'];
                        $row->total_user_cash += $data['user_budan_amount'];
                        $nConsumeCategory     = $data['user_budan_amount'] > 0 ? UserCashLog::CATEGORY_BUDAN_INCR : UserCashLog::CATEGORY_BUDAN_DECR;
                        $nCurrentAmount       = $nLastAmount + $data['user_budan_amount'];
                        $sRemark              = sprintf('%s_%s_补单_%s', date('Y-m-d H:i:s'), __('"现金"'), $data['user_budan_amount']);
                        break;
                    case 'vip':
                        // VIP补单
                        $row->user_member_expire_time = $row->user_member_expire_time < time() ? (time() + $data['user_budan_amount'] * 24 * 3600) : ($row->user_member_expire_time + $data['user_budan_amount'] * 24 * 3600);
                        break;
                    case 'coin':
                    ...
                    default:
                        ...
                        $row->user_budan_total += $data['user_budan_amount'];
