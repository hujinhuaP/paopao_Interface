...
class Invitereward extends Backend
{
    ...
    public function index()
    {
        if ( $this->request->isAjax() ) {
            ...
            $total  = $this->model->where("user_invite_reward_log.recharge_invite_cash = 0")->where($where)->with('User,InviteUser')->join('user invite_user', 'invite_user.user_id=user_invite_reward_log.parent_user_id','inner')->order($sort, $order)->count();
            $list   = $this->model->where("user_invite_reward_log.recharge_invite_cash = 0")->where($where)->with('User,InviteUser')->join('user invite_user', 'invite_user.user_id=user_invite_reward_log.parent_user_id','inner')->order($sort, $order)->limit($offset, $limit)->select();
            $result = [
                "total" => $total,
                "rows"  => $list
            ];
