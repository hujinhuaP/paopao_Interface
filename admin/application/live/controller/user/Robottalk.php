...
class robottalk extends Backend
{
    ...
    public function add()
    {
        ...
        if ( $this->request->isPost() ) {
            ...
            $params['isrobot_talk_content'] = explode("\n", $params['isrobot_talk_content']);
            foreach ($params['isrobot_talk_content'] as $value) {
                if(trim($value) == ''){
                    continue;
                }
                $row                         = new IsrobotTalk();
                $row->isrobot_talk_content   = $value;
                $row->isrobot_talk_user_type = $params['isrobot_talk_user_type'];
                $row->isrobot_talk_type      = $params['isrobot_talk_user_type'] == 'anchor' ? 'female' : 'male';
                $row->validate(
                    [
                        'isrobot_talk_content'   => 'require',
                        'isrobot_talk_type'      => 'require',
                        'isrobot_talk_user_type' => 'require',
                    ],
                    [
                        ...
                        'isrobot_talk_user_type.require' => __('Parameter %s can not be empty', [ 'isrobot_talk_user_type' ]),
                    ]
                );
