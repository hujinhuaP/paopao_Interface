...
class Auth extends \fast\Auth
{
    ...
    public function getChildrenGroupIds($withself = false)
    {
        ...
        // 取出所有分组
        $groupList = model('admin.AuthGroup')->all(['status' => 'normal']);
        $objList = [];
        foreach ($groups as $K => $v)
        {
            if ($v['rules'] === '*')
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard')
    {
        ...
        $select_id = 0;
        // 必须将结果集转换为数组
        $ruleList = collection(model('admin.AuthRule')->where('ismenu', 1)->order('weigh', 'desc')->cache("__menu__")->select())->toArray();
