define(['jquery', 'bootstrap', 'backend', 'table', 'form'], function ($, undefined, Backend, Table, Form) {
    ...
    var Controller = {
            ...
            table.bootstrapTable({
                ...
                columns: [
                    [
                        ...
                        {field: 'live_gift_type', title: __('Reward gift'), searchList: {
                                '1': __('Yes'),
                                '0': __('No'),
                            }, formatter: function (value, row, index) {
                        ...
                        {field: 'operate', title: __('Operate'), table: table,
                            ...
                            formatter: function (value, row, index) {
                                ...
                                +Controller.api.formatter.rewardGift(row.live_gift_type, row, index, 'live_gift_type');
                            }}
        ...
        edit: function () {
            ...
            // 给上传按钮添加上传成功事件
            $("#plupload-live_gift_logo").data("upload-success", function (data) {
                var url = Backend.api.cdnurl(data.url);
                $(".live_gift_logo-img").prop("src", url);
                Toastr.success(__('Upload success'));
            });

            // 给上传按钮添加上传成功事件
            $("#plupload-live_gift_small_gif").data("upload-success", function (data) {
                var url = Backend.api.cdnurl(data.url);
                $(".live_gift_small_gif-img").prop("src", url);
                Toastr.success(__('Upload success'));
            });

            // 给上传按钮添加上传成功事件
            $("#plupload-live_gift_gif").data("upload-success", function (data) {
                var url = Backend.api.cdnurl(data.url);
                $("#c-live_gift_gif2").val(url);
                Toastr.success(__('Upload success'));
            });
        },
        add: function () {
