...
class Log extends Backend {
    ...
    public function history() {
        ...
        if ( $this->request->isAjax() ) {
            ...
            if($error_flg == 'error'){
                $where_str .= ' AND user_private_chat_log.status = 6 AND user_private_chat_log.timepay_count < CEILING(user_private_chat_log.duration/60) AND duration mod 60 > 3 AND user_private_chat_log.create_time > unix_timestamp(\'2018-08-20\')';
            }
            $total  = $this->model::where($where_str)->where($where)->with('User,AnchorUser,UserChatPay')->join('user anchor_user','anchor_user.user_id = user_private_chat_log.chat_log_anchor_user_id')->order($sort, $order)->count();
            $list   = $this->model::where($where_str)->where($where)->with('User,AnchorUser,UserChatPay')->join('user anchor_user','anchor_user.user_id = user_private_chat_log.chat_log_anchor_user_id')->field("'$current_time' as php_current_time")->order($sort, $order)->limit($offset, $limit)->select();
            $result = [
                "total" => $total,
                "rows"  => $list
            ];
