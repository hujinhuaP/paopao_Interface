...
class Stat extends Backend {
    ...
    public function index() {
        if ($this->request->isAjax()) {
            ...
            $total  = $this->model->where($where)->where("agent.first_leader = 0 AND agent.status = 'Y'")->with('Agent')->order($sort, $order)->count();
            $list   = $this->model->where($where)->where("agent.first_leader = 0 AND agent.status = 'Y'")->with('Agent')->order($sort, $order)->limit($offset, $limit)->select();
            $result = [
                "total" => $total,
                "rows"  => $list
            ];
