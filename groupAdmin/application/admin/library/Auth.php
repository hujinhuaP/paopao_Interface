...
class Auth extends \fast\Auth {
    ...
    public function login($username, $password, $keeptime = 0) {
        $admin = GroupAdmin::get(['username' => $username]);
        if (!$admin)
        {
            return false;
        }
        ...
        $admin->save();
        $group = Group::get($admin->group_id)->toArray();
        $admin->group = $group;
    ...
    public function logout() {
        $admin = GroupAdmin::get(intval($this->id));
        if (!$admin)
        {
            return true;
        }
