...
namespace app\helper\TimRestApi;
...
class TimRestAPI extends TimRestInterface
{
	...
	public function openim_send_msg2($account_id, $receiver, $msg_content)
	{
		...
		$msg = array(
				...
				'MsgLifeTime' => 0, //消息离线保存时长（秒数），最长为7天（604800s）。若消息只发在线用户，不想保存离线，则该字段填0。若不填，则默认保存7天
				); 
		#将消息序列化为json串
		$req_data = json_encode($msg);
    ...
    public function querystate($to_account=[])
    {
        ...
        $ret = $this->api("openim", "querystate", $this->identifier, $this->usersig, json_encode($req_data));
        $ret = json_decode($ret, true);
        return $ret;
    }
};
...
function json_format($data, $indent=null)
{
	...
	array_walk_recursive($data, 'app\helper\TimRestApi\json_format_protect');
