...
class WxoauthController extends ControllerBase
{
...
//                $this->log->info("user_id:$user_invite_user_id;group_id: $user_group_id; agent_id: $user_agent_id");
            ...
            if ( $oUserAccount === FALSE ) {
                // 注册微信用户
                $row = $this->registerWX($aUser, $user_invite_user_id, $user_group_id, $user_agent_id, $oUserDeviceBind);
            } else {
                if ( $oUser === FALSE ) {
                    //有一个不为空
            ...
            if ( $is_register == 'Y' ) {
                $flg               = $this->registerFinish($oUserDeviceBind, $sDeviceNo, $sInviteCode, $user_invite_user_id, $user_group_id, $row['user_id'], $user_agent_id);
    ...
    public function webAction()
    {
        $param = http_build_query([
            'appid'         => $this->config->oauth->wx->appid,
            'redirect_uri'  => urlencode($this->config->oauth->wx->callback_url),
            'response_type' => 'code',
            'scope'         => 'snsapi_login',
            'state'         => '1#wechat_redirect',
        ]);

        $url = sprintf('https://open.weixin.qq.com/connect/qrconnect?%s', $param);
    ...
    public function callbackAction()
    {
        ...
        $param = http_build_query([
            ...
            'appid'      => $this->config->oauth->wx->appid,
            'secret'     => $this->config->oauth->wx->appkey,
            'code'       => $code
        ]);

        $url = sprintf('https://api.weixin.qq.com/sns/oauth2/access_token?%s', $param);
        ...
        try {
            ...
            if ( !$aUser || (isset($aUser['errcode']) && $aUser['errcode'] != "") ) {
                throw new Exception($sContent);
            }

            $sOpenId      = $aUser['openid'];
    ...
    protected function getOAuthUserInfo($sOpenId, $sAccessToken)
    {
        ...
        $url = sprintf('https://api.weixin.qq.com/sns/userinfo?%s', $param);
        ...
        if ( isset($aUser['errcode']) && $aUser['errcode'] != 0 ) {
