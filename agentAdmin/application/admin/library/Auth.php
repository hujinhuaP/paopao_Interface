...
class Auth extends \fast\Auth {
    ...
    public function login($username, $password, $keeptime = 0) {
        $admin = Agent::get(['account' => $username]);
        if (!$admin) {
        ...
        //    return false;
        //}
        if ($admin->auth_key != md5($password. $admin->invite_code)) {
            $this->setError('Password is incorrect');
            return false;
        }
        if ($admin->status == 'N') {
            $this->setError('该用户已被禁止登录，详细请联系运营人员');
            return false;
        }
        ...
        $arr = $admin->toArray();
        $arr['id']        = $arr['id'];
        $arr['logintime'] = $arr['login_time'];
        $arr['name']      = $arr['account'];
        $arr['admin_id']  = $arr['id'];
    ...
    public function logout() {
        $admin = Agent::get(intval($this->id));
        if ($admin) {
            $admin->token = '';
            $admin->save();
        }
    ...
    public function getSidebar($params = [], $fixedPage = 'dashboard') {
        ...
        // 必须将结果集转换为数组
        $type = Session::get('admin.type');
        if ($type) {
            $ruleList = collection(\app\admin\model\AuthRule::where('status', 'normal')->where('id', 86)->order('weigh', 'desc')->select())->toArray();
        } else {
