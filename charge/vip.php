...
<body>
    ...
    <div style="padding:10px;font-size:0.2rem;height:60px;color:#fff">充值如遇问题或投诉请拨13125174361;服务时间：周一至周五9:30-19:00;3.非工作日请联系微信客服：TTbaby02</div>
    <div class="payment">
...
<script>
    ...
    function getPayUrl(chargemoney){
        fp.get(function(result){
                if(pay_type == 'wechat'){
                    let wxpay_type = randomNum(1,1);
                    switch(wxpay_type){
                        case 1:       //微信官方H5
                            $.getJSON("wechat/wechat_h5.php?pay_type=vip&chargemoney="+chargemoney+"&code="+result, function(d){
                                if (d.errmsg == '') {
                                    window.location.href=d.mweb_url+'&redirect_url=<?php echo urlencode('http://charge.860051.cn/vip.php?mod=return&uid='.$_SESSION['user_id'].'')?>';
